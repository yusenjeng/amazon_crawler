...
class AmazonCrawler(ProxyCrawler):
    ...
    def parse(self, html, opt):
        ...
        category = soup.find(
            'h4', class_='a-size-small a-spacing-top-mini a-color-base a-text-bold').string

        items = soup.find_all(utility.hasAsin)

        if len(items) < 1:
            self.log.write('0 result for query :' + opt['url'] +
                           ', page = ' + opt['page'])
    ...
    def parseProductPrice(self, item, ad):
        ...
        if whole is None or fraction is None:
            ...
            ad['price'] = float(whole.string + '.' + fraction.string)

        product_id = item.get('data-asin')
        ad['adId'] = product_id

    def getAdsByQuery(self, idx):
        self.switchProxy()

        ad = self.queries[idx]
        opt = {}
        opt['url'] = self.makeQueryUrl(ad['query'])
